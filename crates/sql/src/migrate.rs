...
pub struct AcquiredSchema<DB, C>
where
    DB: Database,
...
impl<DB, C> Deref for AcquiredSchema<DB, C>
where
    DB: Database,
    C: Connection<Database = DB>,
{
...
impl<DB, C> DerefMut for AcquiredSchema<DB, C>
where
    DB: Database,
    C: Connection<Database = DB>,
{
...
impl<DB, C> AcquiredSchema<DB, C>
where
    DB: Database,
    C: Connection<Database = DB>,
{
    pub fn new(connection: C, schema: &'static str) -> Self {
...
impl SchemaMigrator {
    ...
    where
        A: Acquire<'a>,
        <A as Acquire<'a>>::Connection: Connection<Database = A::Database>,
        AcquiredSchema<A::Database, A::Connection>: Migrate,
    {
    ...
    where
        ...
        AcquiredSchema<A::Database, A::Connection>: Migrate,
    {
        let mut conn = AcquiredSchema::<A::Database, A::Connection> {
            connection: migrator.acquire().await?,
            schema: self.schema,
        };
